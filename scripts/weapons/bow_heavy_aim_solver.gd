...
static func get_aim_direction(controller: SafeWeaponController) -> Vector3:
	...
	return direction.normalized() if direction.length_squared() > 0.0001 else _fallback_direction(controller)

static func sample_aim(controller: SafeWeaponController, attack: WeaponAttackDefinition, direction: Vector3) -> Dictionary:
...
static func find_targets(controller: SafeWeaponController, attack: WeaponAttackDefinition, direction: Vector3) -> Array[Node]:
	var targets: Array[Node] = []
	var sample: Dictionary = sample_aim(controller, attack, direction)
	if sample.is_empty():
		return targets
	controller._spawn_ranged_trace(sample.get("origin", Vector3.ZERO) as Vector3, sample.get("end", Vector3.ZERO) as Vector3, attack)
	var target: Node = sample.get("target") as Node
	if target != null:
		targets.append(target)
	return targets

static func _get_reticle_world_point(controller: SafeWeaponController, camera: Camera3D, actor: Node3D) -> Vector3:
	...
	var result: Dictionary = controller.get_world_3d().direct_space_state.intersect_ray(query)
	if not result.is_empty():
		return result.get("position", endpoint) as Vector3
	return endpoint
