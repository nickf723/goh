...
func sample_total_airflow_fast(world_position: Vector3, sample_time: float = -1.0) -> Vector3:
	ensure_field_registry()
	...
	for field_node: Node in registered_fields:
		...
		if contribution is Vector3:
			total_velocity += contribution as Vector3
	fast_sample_count += 1
	return clamp_combined_velocity(total_velocity)
...
func clamp_combined_velocity(total_velocity: Vector3) -> Vector3:
	...
	if speed_limit > 0.0 and total_velocity.length_squared() > speed_limit * speed_limit:
		return total_velocity.normalized() * speed_limit
	return total_velocity
