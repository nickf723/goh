...
The Wilds Expedition is the first exploration-space application of those contracts. Its neutral and regional route assemblies populate a current-bearing Cypress channel with a Goose and Trout, a mossy Wet Woodland climbing snag with a Gecko, and a rooted Pine Ridge burrow with a Mole. A route-local provider exposes Grace, movement noise, bounds, forage/water targets, and same-species alerts to the unchanged generic actors; rebuilds retire the old population and create one clean replacement. Field presentation suppresses laboratory labels without forking the animal runtime.

Do not propose animal movesets, animal personality, bonding, locomotion capability data, runtime ground/swimming/flight/climbing/burrowing steering, active-phase effects, animal vitals, statuses, projectiles, familiar commands, authored Goose/Trout/Gecko/Mole proving species, generic water or traversal habitats, an authored field-habitat adapter, or a generic animal AI foundation as new mechanics. The next extension should add a player-facing observation, feeding, rescue, or ecological interaction to an authored route; ceiling/corner climbing, terrain deformation, population simulation, and broader ecology should be added only when a specific experience proves the need.
