extends Node3D
class_name ThunderstormWeatherSpell


func execute(player: Node3D, _cast_direction: Vector3) -> void:
	var weather_controller: Node = find_thunderstorm_controller()
	if weather_controller == null or not weather_controller.has_method("toggle_weather"):
		show_message("Thunderstorm has nowhere to gather in this scene.")
		queue_free()
		return
...
func find_thunderstorm_controller() -> Node:
	for candidate: Node in get_tree().get_nodes_in_group("weather_controller"):
		...
		if effect_id == "thunderstorm_weather" or weather_kind == "thunderstorm":
			return candidate
	return null
