...
Seed moves cover ambient, forage, retreat, contact, gap-closing, support, projectile, habitat, control, and area-action families, including the reusable Beak-gated Peck. Seed species are Wolf, Sheep, Capybara, Gorgon, Gremlin, Goose, and Trout. The reusable live animal actor presents procedural quadruped, waterfowl, and fish bodies with real vitals, physical effect dispatch, and shared locomotion execution. The canonical Animal Behavior Lab includes a real `SwimmingWaterVolume`, a water-only Trout, a ground/swimming/flying Goose, amphibious Capybara transfer, direct air-water transitions, and controls that restore authored start modes.

Do not propose animal movesets, animal personality, bonding, locomotion capability data, runtime ground/swimming/flight steering, active-phase effect requests, animal vitals, animal status conditions, animal projectiles, familiar commands, authored Goose/Trout proving species, a true animal swim habitat, or a generic animal AI foundation as new mechanics. The next extension is a real climbing animal and a real burrowing animal in existing exploration content, with adhesion and route-topology adapters authored only when those species require them.
