...
func _build_shore_and_causeway() -> void:
	var shore := builder.add_root(authored_root, "ShoreAndCauseway")
	...
	# One continuous deck owns collision. Offset surface slabs create age and breakup without introducing gaps.
	...
	for index: int in range(9):
		...
		builder.add_visual_box(
			...
			Vector3(4.65, 0.16, 3.16),
			Vector3(offset_x, 0.18 + float(index % 2) * 0.025, -2.15 + float(index) * 3.15),
			slab_color,
			Vector3(0.0, yaw, 0.0)
		)
	for side: float in [-1.0, 1.0]:
		for index: int in range(6):
			var post_position := Vector3(side * 2.7, 0.58, -0.5 + float(index) * 4.8)
			var post_height: float = 1.2 if index not in [2, 5] else 0.62
			builder.add_visual_box(
				shore,
				"CausewayPost_%s_%02d" % ["L" if side < 0.0 else "R", index],
				Vector3(0.42, post_height, 0.42),
				post_position,
				"stone_secondary",
				Vector3(0.0, side * 0.08, side * 0.025)
			)
	...
	_build_lantern(shore, "OrinLantern", Vector3(-3.8, 0.0, -3.3), true)


func _build_chapel_shell() -> void:
	var shell := builder.add_root(authored_root, "ChapelShell")

	# Main floor, aisles, and the flooded side chapel are built as contiguous authored surfaces.
	...
	# Subtle wall courses make the large shapes read as masonry rather than monolithic boxes.
	for course: int in range(4):
		var y: float = 0.9 + float(course) * 1.38
		builder.add_visual_box(shell, "WestMortar%02d" % course, Vector3(0.04, 0.055, 14.1), Vector3(-6.68, y, 29.5), "mortar")
		builder.add_visual_box(shell, "EastMortar%02d" % course, Vector3(0.04, 0.055, 14.1), Vector3(6.68, y, 29.5), "mortar")


func _build_architecture() -> void:
	var architecture := builder.add_root(authored_root, "Architecture")
	...
	# Overhead aisle beams imply former arches without narrowing the route at player height.
	...
	for index: int in range(5):
		...
		var broken_tilt: float = -0.08 + float(index % 3) * 0.07
		builder.add_visual_box(architecture, "RafterLeft%02d" % index, Vector3(7.0, 0.22, 0.34), Vector3(-3.25, 6.15 + float(index % 2) * 0.18, z_value), "wood_dark", Vector3(0.0, 0.0, -0.2 + broken_tilt))
		builder.add_visual_box(architecture, "RafterRight%02d" % index, Vector3(7.0, 0.22, 0.34), Vector3(3.25, 6.05, z_value), "wood_dark", Vector3(0.0, 0.0, 0.2 - broken_tilt))
...
func _build_memorial_arcade(parent: Node3D) -> void:
	var arcade := builder.add_root(parent, "MemorialArcade")
	for index: int in range(4):
		...
		if index != 1:
			_build_small_candle(arcade, Vector3(-6.05, 0.82, z_value + 0.35))


func _build_bell_frame(parent: Node3D) -> void:
	var frame := builder.add_root(parent, "BellFrame", Vector3(-1.35, 0.0, 30.65))
	for side: float in [-1.0, 1.0]:
		builder.add_visual_box(frame, "Post%s" % ("L" if side < 0.0 else "R"), Vector3(0.38, 5.4, 0.46), Vector3(side * 1.25, 2.7, 0.0), "wood_dark", Vector3(0.0, 0.0, side * 0.035))
		builder.add_visual_box(frame, "Brace%s" % ("L" if side < 0.0 else "R"), Vector3(0.22, 3.2, 0.28), Vector3(side * 0.72, 2.0, 0.0), "wood_primary", Vector3(0.0, 0.0, -side * 0.42))
...
func _build_altar_and_crypt(parent: Node3D) -> void:
	var altar_root := builder.add_root(parent, "AltarAndCrypt")
...
func _build_rose_window(parent: Node3D) -> void:
	var rose := builder.add_root(parent, "RoseWindow", Vector3(2.65, 4.45, 36.1), Vector3(PI / 2.0, 0.0, 0.0))
	builder.add_visual_torus(rose, "OuterRing", 1.05, 1.18, Vector3.ZERO, "stone_secondary", Vector3.ZERO)
...
func _build_props_and_overgrowth() -> void:
	var props := builder.add_root(authored_root, "PropsAndOvergrowth")
	var pews := builder.add_root(props, "BrokenPews")
	var pew_positions: Array[Vector3] = [
		Vector3(-2.25, 0.0, 25.8),
		Vector3(-2.2, 0.0, 28.0),
		Vector3(-2.15, 0.0, 30.2),
		Vector3(0.05, 0.0, 25.8),
		Vector3(0.05, 0.0, 28.0),
	]
	for index: int in range(pew_positions.size()):
		var yaw: float = 0.0 if index < 3 else 0.04
		var broken: float = -0.45 + float(index) * 0.2
		builder.add_bench(pews, "Pew%02d" % index, pew_positions[index], 2.0 if index != 2 else 1.55, yaw, broken)
...
func _build_water_states() -> void:
	var flooded_visuals := builder.add_root(flooded_state, "FloodPresentation")
	...
	var quiet_visuals := builder.add_root(quiet_state, "QuietPresentation")
...
func _restyle_story_props() -> void:
	...
	if mechanism != null:
		...
		builder.add_visual_box(mechanism, "MechanismHousing", Vector3(2.0, 1.15, 0.8), Vector3(0.0, 0.0, 0.0), "metal_dark", Vector3(0.0, 0.16, 0.0))
...
func _configure_lighting() -> void:
	...
	if directional != null:
		...
		directional.light_color = ChapelPalette.color("moon_light")
	...
	if world_environment != null:
		...
		environment.ambient_light_color = ChapelPalette.color("moon_light")
		environment.ambient_light_energy = 0.48
		environment.fog_enabled = true
		environment.fog_light_color = ChapelPalette.color("fog")
		environment.fog_light_energy = 0.7
		environment.fog_density = 0.012
		world_environment.environment = environment

	var lighting := builder.add_root(authored_root, "Lighting")
...
func _build_reed_cluster(parent: Node3D, node_name: String, position_value: Vector3, count: int) -> void:
	var cluster := builder.add_root(parent, node_name, position_value)
...
func _build_lantern(parent: Node3D, node_name: String, position_value: Vector3, lit: bool) -> void:
	var lantern := builder.add_root(parent, node_name, position_value)
...
func _build_small_candle(parent: Node3D, position_value: Vector3) -> void:
	builder.add_visual_cylinder(parent, "Candle", 0.045, 0.055, 0.24, position_value, Color(0.72, 0.66, 0.48))
	builder.add_visual_sphere(parent, "CandleFlame", 0.05, position_value + Vector3.UP * 0.17, "lantern_light", Vector3(0.65, 1.2, 0.65), 0.8, 1.3, "light_source")
