...
func run_tests() -> void:
	...
	assert_true(menu_shell.get("tab_box") is HBoxContainer, "Field Kit uses horizontal visual tabs")

	menu_shell.call("select_tab", int(menu_shell.call("get_tab_index", "items")))
	menu_shell.set("selected_action_index", 1)
	menu_shell.call("start_item_slot_assignment", "oil_flask")
	assert_equal(str(menu_shell.get("assignment_mode")), "item_slot", "selecting an item opens visual direction placement")
	assert_equal(str(menu_shell.get("action_layout_mode")), "cross", "item-first assignment uses spatial cross navigation")
	var cross_actions: Array = menu_shell.get("selectable_actions")
	assert_equal(cross_actions.size(), 4, "visual belt picker exposes four directions")
	menu_shell.set("selected_action_index", 0)
	menu_shell.call("select_action_direction", -1, 0)
	assert_equal(int(menu_shell.get("selected_action_index")), 1, "cross navigation maps left input to Left belt slot")
	menu_shell.call("cancel_assignment")
	assert_equal(int(menu_shell.get("selected_tab_index")), int(menu_shell.call("get_tab_index", "items")), "item-first Cancel returns to inventory grid")
	assert_equal(int(menu_shell.get("selected_action_index")), 1, "item-first Cancel restores selected inventory tile")
	menu_shell.call("start_item_slot_assignment", "oil_flask")
	menu_shell.call("complete_item_slot_assignment", PlayerQuickItemController.SLOT_DOWN)
	assert_equal(GameState.get_quick_item_slot(PlayerQuickItemController.SLOT_DOWN), "oil_flask", "visual belt picker assigns selected item")
