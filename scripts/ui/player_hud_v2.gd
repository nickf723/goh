...
var dialogue_preview_active: bool = false
...
func _build_stats_panel() -> void:
	...
	avatar_title_label.text = "GRACE  •  WAYFARER"
	...
	_create_stat_row(stack, "health", "♥", "HEALTH", Color(0.94, 0.18, 0.27))
	_create_stat_row(stack, "mana", "◆", "MANA", Color(0.18, 0.58, 1.0))
	_create_stat_row(stack, "stamina", "✦", "STAMINA", Color(0.31, 0.82, 0.4))
	_create_stat_row(stack, "stance", "◇", "STANCE", Color(0.96, 0.71, 0.18))
...
func _build_quick_panel() -> void:
	...
	heading.add_theme_color_override("font_color", Color(0.73, 0.82, 1.0, 0.92))
	...
	spell_row.add_child(_make_arrow_label("◀"))
	...
	var item_panel := _make_quick_subpanel(
	...
	quick_item_label.add_theme_color_override("font_color", Color(0.72, 0.88, 1.0))
	item_panel.add_child(quick_item_label)

	var special_panel := _make_quick_subpanel(
		Color(0.055, 0.027, 0.015, 0.92),
		Color(1.0, 0.43, 0.08, 0.62)
	)
	lower_row.add_child(special_panel)
	var special_stack := VBoxContainer.new()
	...
	divine_special_label.add_theme_color_override("font_color", Color(1.0, 0.68, 0.24))
	...
	divine_charge_bar.add_theme_stylebox_override(
		"fill",
		_make_bar_style(Color(1.0, 0.47, 0.08), 4)
	)
	...
	quick_hint_label.text = (
		"D-PAD ◀/▶ SPELLS   •   ▲ TAP CYCLE / HOLD USE   •   "
		+ "▼ TAP ACTIVATE / HOLD SELECT"
	)
	...
	quick_hint_label.add_theme_color_override("font_color", Color(0.56, 0.65, 0.78, 0.88))
	stack.add_child(quick_hint_label)


func _make_arrow_label(copy: String) -> Label:
	var label := Label.new()
	label.text = copy
	label.add_theme_font_size_override("font_size", 18)
	label.add_theme_color_override("font_color", Color(0.55, 0.7, 1.0, 0.9))
	return label


func _make_quick_subpanel(fill: Color, border: Color) -> PanelContainer:
	var panel := PanelContainer.new()
	panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	panel.custom_minimum_size = Vector2(0.0, 61.0)
	panel.add_theme_stylebox_override(
		"panel",
		_make_panel_style(fill, border, 11, 1)
	)
	return panel
...
func _build_portrait_and_statuses() -> void:
	...
	for _index: int in range(5):
		...
		effect_name.add_theme_color_override("font_color", Color(0.9, 0.94, 1.0, 0.96))
		effect_row.add_child(effect_name)
		status_name_labels.append(effect_name)

		var effect_timer := Label.new()
	...
	portrait_shell.add_child(margin)
	...
	portrait_name_label.add_theme_color_override("font_color", Color(1.0, 0.78, 0.36))
	stack.add_child(portrait_name_label)
	...
	portrait_state_label.add_theme_color_override("font_color", Color(0.64, 0.74, 0.88, 0.9))
	stack.add_child(portrait_state_label)
...
func _build_dialogue_panel() -> void:
	...
	npc_portrait = PortraitMedallionScript.new()
	...
	dialogue_speaker_label.add_theme_color_override("font_color", Color(1.0, 0.73, 0.28))
	...
	dialogue_title_label.add_theme_color_override("font_color", Color(0.55, 0.65, 0.78, 0.92))
	...
	dialogue_body_label.add_theme_color_override("default_color", Color(0.92, 0.94, 0.98, 0.98))
	...
	dialogue_choice_label.add_theme_color_override("font_color", Color(1.0, 0.76, 0.34, 0.96))
...
func _refresh_stats() -> void:
	...
	if avatar_manager != null:
		if avatar_manager.has_method("get_active_avatar_display_name"):
			avatar_name = str(avatar_manager.call("get_active_avatar_display_name"))
	...
	base_stats_label.text = (
		...
		+ "   POWER " + str(GameState.get_stat("power"))
		+ "   ARCANA " + str(GameState.get_stat("arcana"))
	)
...
func _update_stat_row(stat_id: String, maximum_id: String) -> void:
	if not stat_rows.has(stat_id):
		return
	var row_value: Variant = stat_rows[stat_id]
	if not row_value is Dictionary:
		return
	var row: Dictionary = row_value
	var bar_value: Variant = row.get("bar")
	var label_value: Variant = row.get("value")
	...
	if bar_value is ProgressBar:
		var bar: ProgressBar = bar_value
		bar.max_value = maximum
		bar.value = clampi(current, 0, maximum)
	if label_value is Label:
		var value_label: Label = label_value
...
func _refresh_quick_loadout() -> void:
	...
	if control_router != null:
		if control_router.has_method("get_quick_spell_names"):
			var names_result: Variant = control_router.call("get_quick_spell_names")
		...
		if control_router.has_method("get_selected_quick_spell_cursor"):
			spell_cursor = int(control_router.call("get_selected_quick_spell_cursor"))
	spell_cursor = clampi(spell_cursor, 0, 2)

	for index: int in range(quick_spell_labels.size()):
		var spell_name: String = "Empty"
		if index < spell_names.size():
			spell_name = spell_names[index]
		quick_spell_labels[index].text = spell_name
		var selected: bool = index == spell_cursor and spell_name != "Empty"
	...
	if control_router != null and control_router.has_method("get_selected_quick_item_slot"):
		selected_item_slot = int(control_router.call("get_selected_quick_item_slot"))
	var item_name: String = "Empty"
	var item_count: int = 0
	if quick_item_controller != null:
		var item: QuickItemDefinition = quick_item_controller.get_slot_item(selected_item_slot)
		if item != null:
			item_name = item.display_name
			item_count = quick_item_controller.get_slot_charges(selected_item_slot)
	...
	if divine_controller != null:
		...
		maximum_charge = maxf(float(debug.get("maximum_charge", 100.0)), 1.0)
	...
	divine_charge_label.text = str(roundi(charge / maximum_charge * 100.0)) + "% " + state
...
func _refresh_statuses_and_portrait() -> void:
	...
	for index: int in range(status_rows.size()):
		...
		status_rows[index].visible = has_row
		if not has_row:
			continue
		...
		status_name_labels[index].text = str(row.get("name", status_id.capitalize()))
		var remaining: float = maxf(float(row.get("remaining", 0.0)), 0.0)
		status_timer_labels[index].text = str(ceili(remaining)) + "s" if remaining > 0.0 else ""
		status_timer_labels[index].add_theme_color_override("font_color", color)

	last_expression = _resolve_portrait_expression(rows)
	portrait.configure("Grace", _get_portrait_accent(), last_expression, true)
	var health: int = GameState.get_stat("health")
	var maximum_health: int = maxi(GameState.get_stat("max_health"), 1)
	portrait_name_label.text = "GRACE"
	portrait_state_label.text = _portrait_state_copy(last_expression, health, maximum_health)


func _collect_status_rows() -> Array[Dictionary]:
	var rows: Array[Dictionary] = []
	var seen: Dictionary = {}
	var effect_rows: Variant = GameplayEffectAccessScript.get_active_source_rows(false)
	if effect_rows is Array:
		for effect_value: Variant in effect_rows:
			if not effect_value is Dictionary:
				continue
			var effect_row: Dictionary = effect_value
	...
	if status_receiver != null:
		for status_value: Variant in status_receiver.active_statuses.keys():
			...
			if seen.has(status_id):
				continue
			var status_data_value: Variant = status_receiver.active_statuses[status_id]
			if not status_data_value is Dictionary:
				continue
			var status_data: Dictionary = status_data_value
			seen[status_id] = true
			rows.append({
				"id": status_id,
				"name": status_id.capitalize(),
				"remaining": float(status_data.get("duration", 0.0)),
			})

	if divine_controller != null:
		var divine_debug: Dictionary = divine_controller.get_debug_data()
		if bool(divine_debug.get("ready", false)) and not seen.has("divine_ready"):
			rows.push_front({
				"id": "divine_ready",
	...
	for index: int in range(mini(rows.size(), 5)):
		limited.append(rows[index])
	return limited


func _resolve_portrait_expression(rows: Array[Dictionary]) -> String:
	if active_conversation != null or dialogue_preview_active:
		return "attentive"
	...
	if "poisoned" in status_ids:
		return "poisoned"
	if "chill" in status_ids or "chilled" in status_ids or "frozen" in status_ids:
		return "chilled"
	var maximum_health: int = maxi(GameState.get_stat("max_health"), 1)
	if GameState.get_stat("health") <= ceili(float(maximum_health) * 0.25):
		return "low_health"
	if action_state != null and (action_state.is_casting or action_state.is_focus_menu_open):
		return "focused"
	return "neutral"


func _portrait_state_copy(expression: String, health: int, maximum_health: int) -> String:
	match expression:
		"attentive":
			return "LISTENING"
		"burning":
...
func _refresh_dialogue() -> void:
	...
	if conversation_data_value is Dictionary:
		conversation_data = conversation_data_value
	...
	var accent: Color = Color(0.78, 0.58, 0.28)
	...
	dialogue_title_label.text = str(conversation.get("title"))
	dialogue_body_label.text = str(node.get("text", "..."))
	...
	if choices.is_empty():
		dialogue_choice_label.text = "A  Continue     B  Leave"
		return
	var selected_choice: int = clampi(
		int(conversation.get("selected_choice")),
		0,
		choices.size() - 1
	)
	...
	if selected_value is Dictionary:
		selected_text = str(selected_value.get("text", "Continue"))
	dialogue_choice_label.text = (
		"◆  " + selected_text + "     ▲/▼ Choose   A Confirm   B Leave"
	)


func show_dialogue_preview(data: Dictionary) -> void:
	dialogue_preview_active = true
	active_conversation = null
	dialogue_panel.visible = true
	dialogue_speaker_label.text = str(data.get("speaker", "WAYFARER")).to_upper()
	dialogue_title_label.text = str(data.get("title", "Traveler"))
	dialogue_body_label.text = str(data.get("text", "..."))
	dialogue_choice_label.text = str(data.get("choice", "A  Continue     B  Leave"))
	var accent: Color = Color(0.78, 0.58, 0.28)
	var accent_value: Variant = data.get("accent", accent)
	if accent_value is Color:
		accent = accent_value
	npc_portrait.configure(str(data.get("name", "Traveler")), accent, "neutral", false)
	_refresh_statuses_and_portrait()
...
func _suppress_legacy_hud() -> void:
	...
	if game_ui == null:
		return
	for child_name: String in ["ResourceHUD", "SpellMenuLabel", "DebugStatsLabel"]:
...
func _resolve_bindings() -> void:
	...
	status_receiver = actor.get_node_or_null("StatusReceiver") as PlayerStatusReceiver


func _get_hand_role_summary() -> Dictionary:
	if control_router != null and control_router.has_method("get_hand_role_summary"):
		var result: Variant = control_router.call("get_hand_role_summary")
		if result is Dictionary:
			return result
	return {"focus": "L", "cast": "ZL", "light": "R", "heavy": "ZR"}
...
func _status_color(status_id: String) -> Color:
	match status_id:
		"burning":
			return Color(1.0, 0.34, 0.1)
		"poisoned":
			return Color(0.48, 0.94, 0.22)
		"chill", "chilled", "frozen":
		...
		"vital_restoration":
			return Color(1.0, 0.38, 0.5)
		"resonant_focus":
			return Color(0.3, 0.8, 1.0)
		_:
			return Color(0.72, 0.8, 0.94)
