...
func _update_charge_state(delta: float) -> void:
	...
	if equipped_weapon == null:
		...
		return
	var expected: Dictionary = ChargeCatalogScript.get_profile(equipped_weapon.weapon_class, charge_input_kind)
	if expected.is_empty() or str(expected.get("id", "")) != str(charge_profile.get("id", "")):
		_cancel_weapon_charge()
		return
	charge_elapsed += maxf(delta, 0.0)
	if charge_pending:
		var threshold: float = float(charge_profile.get("threshold", 0.25))
		if charge_elapsed >= threshold:
...
func update_current_attack(delta: float) -> void:
	if charge_active and current_attack != null and current_attack.extra_tags.has("weapon_charge_hold"):
...
func is_weapon_charge_active() -> bool:
