...
# These are development-facing magical relationships, not mutually exclusive
# classes. Their labels can later receive setting-specific names without
# changing the stable progression IDs.
const TRADITIONS: Dictionary = {
	...
	"druidry": {
		...
		"compatibility_tags": [
			"nature", "environmental_source", "terrain", "plant", "creature",
			"weather", "growth", "ecosystem",
		],
		"compatibility_categories": [],
		"capstone": {
	...
	"warlock": {
		...
		"compatibility_tags": [
			"patron", "divine", "covenant", "invocation", "manifestation", "incarnation",
		],
		"compatibility_categories": ["summon"],
		"capstone": {
			"id": "divine_incarnation",
			"display_name": "Divine Incarnation",
			"description": "Manifest a patron god as the active playable avatar through Grace's stable world anchor.",
			"implemented": true,
		},
	},
	"theurgy": {
		...
		"compatibility_tags": [
			"heal", "healing", "protection", "ward", "blessing", "revival",
			"cleanse", "support",
		],
		"compatibility_categories": [],
		"capstone": {
	...
	"bardic": {
		...
		"compatibility_tags": [
			"resonance", "echo", "rhythm", "performance", "song", "emotion", "cadence",
		],
		"compatibility_categories": [],
		"capstone": {
	...
	"artifice": {
		...
		"compatibility_tags": [
			"device", "gadget", "trap", "construct", "automaton", "imbue",
			"stored_spell", "machinery",
		],
		"compatibility_categories": [],
		"capstone": {
	...
	"ritualism": {
		...
		"compatibility_elements": [],
		"compatibility_tags": [
			"ritual", "circle", "delayed", "persistent", "prepared", "large_scale", "ceremony",
		],
...
static func get_icon(tradition_id: String) -> String:
	return str(get_definition(tradition_id).get("icon", "✦"))


static func get_baseline_stage_id(tradition_id: String) -> String:
	return str(get_definition(tradition_id).get("baseline_stage_id", ""))


static func get_baseline_tradition_ids() -> Array[String]:
	var tradition_ids: Array[String] = []
	for tradition_id: String in TRADITION_IDS:
		if get_baseline_stage_id(tradition_id) != "":
			tradition_ids.append(tradition_id)
...
static func get_achievement_definition(
	tradition_id: String,
	stage_id: String
) -> Dictionary:
	...
	return {
		...
		"description": _get_stage_description(tradition_id, stage_id),
		"type": "achievement",
...
static func validate_catalog() -> Array[String]:
	...
	for tradition_id: String in TRADITION_IDS:
		...
		if str(definition.get("icon", "")).strip_edges() == "":
			errors.append(tradition_id + " has no icon")
		...
		var baseline_stage_id: String = str(definition.get("baseline_stage_id", ""))
		if baseline_stage_id != "" and not has_stage(baseline_stage_id):
			errors.append(tradition_id + " has an invalid baseline stage")
		...
		if not capstone.get("implemented", null) is bool:
			errors.append(tradition_id + " capstone must declare implemented state")
	...
	if get_baseline_tradition_ids() != ["sorcery", "wizardry"]:
		errors.append("story baseline must initiate Sorcery and Wizardry only")
	if not bool(get_capstone("warlock").get("implemented", false)):
		errors.append("Warlock capstone must reflect the implemented Divine Incarnation proxy")
