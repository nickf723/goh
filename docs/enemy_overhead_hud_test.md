...
- The HUD floats above the enemy and faces the camera.
- It shows:
  - A health bar.
...
6. Lock on with `R3` / `T` and cast spells.
7. Confirm the health and stance bars update after hits.
8. Apply statuses:
   - Poison Cloud should show `P`.
   - Fire Field should show `F`.
   - Ice Lance / chill effects should show `C` or `❄` where applicable.
   - Lightning effects should show `⚡` when stunned.
9. Wait for statuses to expire and confirm icons vanish.
...
- Some special symbols may render as fallback boxes on certain fonts/platforms. If that happens, swap them to plain letters.
