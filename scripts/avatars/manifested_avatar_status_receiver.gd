...
func _ready() -> void:
	...
	add_to_group("manifested_avatar_status_receiver")
...
func apply_status(
	...
	var normalized: String = status_name.strip_edges().to_lower()
	if normalized == "" or duration <= 0.0:
		return
	var source_element: String = _get_status_element(normalized)
	if (
		authority_controller != null
		and not authority_controller.can_receive_status(normalized, source_element)
	):
		last_result = "blocked:" + normalized
		total_blocked_statuses += 1
		status_blocked.emit(normalized, source)
		return
	_resolve_status_conflicts(normalized)
...
func _apply_status_tick(status_name: String, status: Dictionary) -> void:
	...
	payload.tags = [payload.element, status_name, "status", "manifestation_status"]
	if authority_controller != null:
