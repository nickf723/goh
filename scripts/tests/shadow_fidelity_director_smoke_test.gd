...
func _validate_cinematic(shadows: ShadowFidelityDirector3D) -> void:
	...
	_expect(int(data.get("sun_shadow_mode", -1)) == DirectionalLight3D.SHADOW_PARALLEL_4_SPLITS, "Cinematic sun keeps four PSSM splits")
...
func _validate_balanced(
	...
	_expect(int(data.get("sun_shadow_mode", -1)) == DirectionalLight3D.SHADOW_PARALLEL_4_SPLITS, "Balanced retains four PSSM splits")
