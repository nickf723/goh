...
func run_tests() -> void:
	...
	_expect(
		target is PrototypeGreenGrottoShadowPass,
		"Green Grotto installs Shadow Fidelity integration"
	)
	_expect(
		str(target.get_meta("shadow_fidelity_authority", ""))
		== "ShadowFidelityDirector",
		"Green Grotto declares ShadowFidelityDirector authority"
	)

	var lighting: LightingDirector3D = target.get_node_or_null(
		"LightingDirector"
	) as LightingDirector3D
	var shadows: ShadowFidelityDirector3D = target.get_node_or_null(
		"ShadowFidelityDirector"
...
func _validate_contract(shadows: ShadowFidelityDirector3D) -> void:
	...
	_expect(int(data.get("accent_lights", 0)) == 2, "Director resolves the two Green local accent lights")
	_expect(int(data.get("managed_foliage", 0)) >= 350, "Director resolves dense foliage shadow casters")
...
func _validate_cinematic(shadows: ShadowFidelityDirector3D) -> void:
	...
	_expect(int(data.get("directional_atlas_size", 0)) == 8192, "Cinematic tier uses large directional atlas")
	_expect(int(data.get("positional_atlas_size", 0)) == 4096, "Cinematic tier budgets local shadow atlas")
	_expect(int(data.get("filter_quality", -1)) == 4, "Cinematic tier uses high soft-shadow filtering")
	_expect(int(data.get("sun_shadow_mode", -1)) == DirectionalLight3D.SHADOW_PARALLEL_4_SPLITS, "Cinematic sun uses four PSSM splits")
	_expect(absf(float(data.get("sun_shadow_distance", 0.0)) - 94.0) < 0.1, "Cinematic tier keeps useful long-range shadows")
	_expect(float(data.get("sun_bias", 1.0)) < 0.05, "Cinematic tier tightens shadow bias for grounding")
	_expect(float(data.get("sun_normal_bias", 9.0)) < 0.8, "Cinematic tier tightens normal bias")
	_expect(float(data.get("sun_angular_distance", 0.0)) > 0.45, "Cinematic sun uses contact-hardening angular size")
	_expect(int(data.get("accent_shadow_lights", 0)) == 2, "Cinematic tier enables both local accent shadow maps")
	_expect(int(data.get("double_sided_foliage_shadows", 0)) >= 350, "Cinematic tier enables double-sided foliage shadowing")
...
func _validate_performance(
	...
	_expect(int(data.get("sun_shadow_mode", -1)) == DirectionalLight3D.SHADOW_PARALLEL_2_SPLITS, "Performance tier drops to two sun splits")
	_expect(absf(float(data.get("sun_shadow_distance", 0.0)) - 46.0) < 0.1, "Performance tier shortens shadow range")
	...
	_expect(int(data.get("double_sided_foliage_shadows", 99)) == 0, "Performance tier restores cheap single-sided foliage shadowing")
...
func _validate_balanced(
	...
	_expect(int(data.get("directional_atlas_size", 0)) == 4096, "Balanced tier restores standard directional atlas")
	_expect(int(data.get("positional_atlas_size", 0)) == 2048, "Balanced tier restores moderate positional atlas")
	_expect(int(data.get("filter_quality", -1)) == 3, "Balanced tier restores medium soft filtering")
	_expect(int(data.get("sun_shadow_mode", -1)) == DirectionalLight3D.SHADOW_PARALLEL_4_SPLITS, "Balanced tier restores four PSSM splits")
	_expect(float(data.get("sun_angular_distance", 0.0)) > 0.2, "Balanced tier enables restrained contact-hardening")
	_expect(int(data.get("accent_shadow_lights", 99)) == 0, "Balanced tier still avoids local accent shadows")
	var double_sided: int = int(data.get("double_sided_foliage_shadows", 0))
	var managed: int = int(data.get("managed_foliage", 0))
	_expect(double_sided > 0, "Balanced tier gives close foliage double-sided shadows")
	_expect(double_sided < managed, "Balanced tier keeps canopy on cheaper shadow policy")
...
func _validate_cinematic_restore(
	...
	_expect(int(data.get("directional_atlas_size", 0)) == 8192, "Cinematic atlas returns after tier cycle")
	_expect(int(data.get("accent_shadow_lights", 0)) == 2, "Cinematic local shadows return after tier cycle")
	_expect(int(data.get("double_sided_foliage_shadows", 0)) == int(data.get("managed_foliage", -1)), "Cinematic foliage shadow policy returns after tier cycle")
...
func _validate_layer_independence(
	...
	var vegetation: VegetationPresentationDirector3D = target.get_node_or_null(
