...
func try_threat_response(delta: float) -> bool:
	...
	var threat: CombatThreat = threat_sensor.get_best_actionable_threat(actor.global_position)
	if threat == null:
		current_threat = null
	...
	var response: EnemyActionOption = select_threat_response(get_distance_to_player(), threat)
	if response == null:
...
func select_threat_response(distance: float, threat: CombatThreat) -> EnemyActionOption:
	...
	for option: EnemyActionOption in action_options:
		...
		score += threat.severity * max(threat_score_severity_scale, 0.0)
		if score <= best_score:
			continue
...
func commit_threat_response(
	...
	last_action_summary = "reacting to " + threat.display_name + " with " + option.get_display_name()

	var default_commit_time: float = max(get_definition().get_attack_commit_time(), 0.0)
	default_commit_time *= get_personality_number("attack_commit_time_multiplier", 1.0)
...
func get_debug_data() -> Dictionary:
	...
	debug_data["threat_time"] = (
		snapped(current_threat.get_time_until_impact(), 0.01)
