...
func update_timers(delta: float) -> void:
	...
	if strafe_switch_timer <= 0.0:
		strafe_switch_timer = get_definition().strafe_switch_interval
...
func process_idle(_delta: float) -> void:
	...
	if get_distance_to_player() <= get_definition().detection_radius:
...
func process_chase(delta: float) -> void:
	...
	if distance > get_definition().lose_interest_radius:
		change_state(EnemyState.IDLE)
		return
	...
	if default_attack != null:
		...
		if distance <= get_definition().preferred_distance + get_definition().spacing_buffer:
...
func process_attack_recover(_delta: float) -> void:
	clear_horizontal_velocity()

	state_timer -= _delta

	if state_timer <= 0.0:
		if player != null and get_distance_to_player() <= get_definition().lose_interest_radius:
			change_state(EnemyState.CHASE)
		else:
			change_state(EnemyState.IDLE)
...
func perform_attack() -> void:
	...
	if not is_player_in_attack_range():
		...
		if default_attack.show_miss_message:
			show_message(get_definition().display_name + " misses.")
...
func move_toward_player(delta: float) -> void:
	...
	var speed: float = get_definition().move_speed * move_multiplier
...
func face_direction(direction: Vector3, delta: float) -> void:
	...
	var turn_amount: float = clamp(get_definition().turn_speed * delta, 0.0, 1.0)
...
func apply_gravity(delta: float) -> void:
	if not actor.is_on_floor():
		actor.velocity.y -= get_definition().gravity * delta
	else:
...
func apply_definition_tags() -> void:
	if enemy_definition == null:
		return
	...
	for tag: String in enemy_definition.tags:
...
func get_debug_data() -> Dictionary:
	return {
		...
		"enemy": get_definition().display_name,
...
func wait_for_attack_opening(delta: float) -> void:
	...
	if get_definition().circle_when_waiting_to_attack:
		circle_player(delta)
	else:
		clear_horizontal_velocity()
		face_player(delta)
...
func circle_player(delta: float) -> void:
	...
	var desired_distance: float = get_definition().preferred_distance
	var distance: float = get_distance_to_player()
	var spacing_push: Vector3 = Vector3.ZERO

	if distance < desired_distance:
		spacing_push = -toward_player * 0.65
	elif distance > desired_distance + get_definition().spacing_buffer:
		spacing_push = toward_player * 0.35

	var move_direction: Vector3 = (tangent + spacing_push).normalized()
	var move_multiplier: float = get_status_move_multiplier()
	var speed: float = get_definition().move_speed * get_definition().strafe_speed_multiplier * move_multiplier
